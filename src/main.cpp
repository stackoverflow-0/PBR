...
bool follow = true;
glm::vec3 FollowcameraPos   = glm::vec3(0.0f, 5.0f, 1.0f);
glm::vec3 FollowcameraFront = glm::vec3(0.0f, 0.0f, -1.0f);
glm::vec3 FollowcameraUp    = glm::vec3(0.0f, 1.0f,  0.0f);
float FollowYawAdjust = 0.0f;
float FollowPitchAdjust = 0.0f;
float world_scale_rate = 5.0f;
// glm::vec3 boat_v = glm::vec3(0,0,1);
glm::vec3 boat_init_v = glm::vec3(0,0,1);
glm::vec3 boat_left = glm::vec3(-1,0,0);
glm::vec3 boat_up = glm::vec3(0,1,0);
glm::vec3 boat_pos = glm::vec3(0,world_scale_rate + 0.05f,0);

float collide_distance = 1.25f;

glm::vec3 particle_pos = glm::vec3(-0.3,0,-0.7);
Particle_Flow p1 = Particle_Flow(particle_pos,glm::vec3(1,1,1),glm::vec3(0,0.7,-1),1,200);
Particle_Flow p2 = Particle_Flow(particle_pos+glm::vec3(0.2,0,2),glm::vec3(1,1,1),glm::vec3(0.5,0.7,1),1,200);
Particle_Flow p3 = Particle_Flow(particle_pos,glm::vec3(-0.2,0,2),glm::vec3(0.5,0.7,1),1,200);
// glm::mat4 boat_rotate_mat = glm::mat4(1.0f);

float speed = 0.0f;
float max_speed = 0.4f;
float collide_delta_angle = 0.0f;
float angle = 0.0f;
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos) {
    ...
    float yaw_mapped, pitch_mapped;
    if(follow){
        yaw_mapped = tanh(yaw * 0.01f) * 1.0f;
        pitch_mapped = -1 * tanh(pitch * 0.01f) * 2.0f;
        if(pitch_mapped > 89.0f) pitch_mapped =  89.0f;
        if(pitch_mapped < -89.0f) pitch_mapped = -89.0f;
        FollowYawAdjust = yaw_mapped;
        FollowPitchAdjust = pitch_mapped;
    }
...
void input_process(GLFWwindow* window) {
    ...
    angle += collide_delta_angle;
    delta_angle += collide_delta_angle;
    collide_delta_angle = 0.0f;

    if (glfwGetKey(window,GLFW_KEY_F) == GLFW_PRESS && cd >= 0.1f){
        follow = !follow;
        cd = 0.0f;
    }
    ...
    FollowcameraFront = glm::normalize(glm::vec3(5.0f*sin(angle - FollowYawAdjust), -2.0f - FollowPitchAdjust, 5.0f*cos(angle - FollowYawAdjust)));
    FollowcameraPos   = boat_pos - FollowcameraFront * 5.0f;
}
...
int main() {
    ...
    */
    ...
    Model sea("./resource/model/NM_sphere.obj");
    Model boat("./resource/model/boat.obj");
    boat.export_mesh("./resource/export/boat.obj");
    Model lighthouse("./resource/model/lighthouse.obj");

    glm::vec3 objectColor = glm::vec3(0.8,0.8,0.8);

    glm::vec3 lightColor = glm::vec3(1.0,1.0,1.0);

    glm::vec3 lightPos = glm::vec3(0.0, world_scale_rate + 10.0f,0.0);
    ...
    auto skybox = Model("./resource/model/HD_sphere.obj");
    ...
    auto suntexture = Texture("./resource/texture/Water_COLOR.jpg");
    auto boat_texture = Texture("./resource/texture/boat.jpg");
    // auto water_height = Texture("./resource/texture/Water.jpg");
    auto water_height = Texture("./resource/water_dis/Calm_loop_tile_01001.jpg");
    auto lighthouse_texture = Texture("./resource/texture/lighthouse.jpg");
    // auto water_height = Texture("./resource/water_dis_all.jpg");
    ...
    */ 
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        sprintf(water_dis_path,"./resource/water_dis/Calm_loop_tile_01%03d.jpg",wave_frame_id);
        water_height.update_texture(water_dis_path);
        ...
        glm::mat4 projection;
        if(follow){
            view = glm::lookAt(FollowcameraPos, FollowcameraPos + FollowcameraFront, FollowcameraUp); 
            projection = glm::perspective(glm::radians(45.0f), SCR_WIDTH*1.0f / SCR_HEIGHT, 0.1f, 100.0f);
            shader.setVec3("viewPos",FollowcameraPos);
        }
        else{
        ...
        */
        ...
        glm::mat4 lightView = glm::lookAt(
            glm::vec3(sea_rotate_mat * glm::vec4(0,world_scale_rate + 10.0f,0,1)),//lightPos, 
